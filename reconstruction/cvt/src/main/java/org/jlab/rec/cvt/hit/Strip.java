...
public class Strip {
	...
	public void calc_BMTStripParams(org.jlab.rec.cvt.bmt.Geometry geo, int sector, int layer) {
		...
		if(org.jlab.rec.cvt.bmt.Geometry.getZorC(layer)==1) { // Z-detectors
			...
			double sigma = org.jlab.rec.cvt.bmt.Constants.SigmaMax/Math.sqrt(Math.cos(org.jlab.rec.cvt.bmt.Constants.getThetaL())); // max sigma for drift distance  (hDrift) = total gap from top to mesh
			
			int num_region = (int) (layer+1)/2 - 1; // region index (0...2) 0=layers 1&2, 1=layers 3&4, 2=layers 5&6double Z0=0;
			//max phi err
			double phiErr = (sigma/Math.cos(org.jlab.rec.cvt.bmt.Constants.getThetaL())
					-(org.jlab.rec.cvt.bmt.Constants.hDrift-org.jlab.rec.cvt.bmt.Constants.getCRZRADIUS()[num_region])*Math.tan(org.jlab.rec.cvt.bmt.Constants.getThetaL()))/org.jlab.rec.cvt.bmt.Constants.getCRZRADIUS()[num_region];
