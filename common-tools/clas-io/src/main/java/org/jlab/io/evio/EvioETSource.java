...
public class EvioETSource implements DataSource {
    ...
    public void open(String filename) {
        LOGGER.debug("[ETSOURCE] -->>> connecting to host : [" +
                this.etRingHost + "]  FILE [" + filename + "]  PORT [" +
                this.etRingPort + "]");
        LOGGER.debug("[ETSOURCE] -->>> connecting remotely : " + this.remoteConnection);
        try {
            ...
            //LOGGER.debug("-------------->>>>> CONNECTING LOCALY");
            ...
            //LOGGER.debug("[ET-RING] ----> opened a stream with events # = " + this.readerEvents.size());
    ...
    public void loadEvents(){
        ...
        if(this.connectionOK == false){
            LOGGER.debug("[EvioETSource] ---->  connection was not estabilished...");
        }
                ...
                if(events!=null){
                    ...
                    if(events.length>0){
                        ...
                        for(int nevent = 0; nevent < events.length; nevent++){
                            ...
                            //EvioReader reader;
                            //LOGGER.debug("------> parsing event # " + nevent +
                            //        " width length = " + length);
                            try {
                                ...
                                LOGGER.debug("----> rawbytes size = " + event.getRawBytes().length);
                                for(BaseStructure base : nodes){
                                    LOGGER.debug("----> event # " + nevent + " " + base);
                                }*/
    ...
    public static void main(String[] args){
        ...
        LOGGER.debug("[ET] PARSING ARGUMENTS");
        parser.parse(args);

        LOGGER.debug("[ET] STARTING DEBUG");
        ...
        while(true){
            ...
            while(reader.hasEvent()==false){
                ...
                LOGGER.debug("[ET] READING ET RING ITERATION # " + iteration);
                iteration++;
            }
            LOGGER.debug("[ET] >> LOADED EVENTS FROM ET RING. COUNT = " + reader.getSize());
            LOGGER.debug("[ET] >> DON'T WORRY EVERYTHING IS FINE. JUST SLEEPING FOR A WHILE. CYCLE # "
                    + cycle + "\n");
