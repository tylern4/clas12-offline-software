...
public class DatabaseConstantProvider implements ConstantProvider {
    ...
    private String getEnvironment(){
        ...
        //LOGGER.debug("ENVIRONMENT : " + envCLAS12 + " " + envCCDB + " " + propCLAS12 + " " + propCCDB);
    ...
    private void initialize(String address){
        ...
        if(debugMode>0){
            LOGGER.debug("[DB] --->  open connection with : " + address);
            LOGGER.debug("[DB] --->  database variation   : " + this.variation);
            LOGGER.debug("[DB] --->  database run number  : " + this.runNumber);
            LOGGER.debug("[DB] --->  database time stamp  : " + databaseDate);
        }

        provider.connect();

        if(provider.isConnected()==true){
            if(debugMode>0) LOGGER.debug("[DB] --->  database connection  : success");
        } else {
            LOGGER.debug("[DB] --->  database connection  : failed");
        }
    ...
    public final void setTimeStamp(String timestamp){
        ...
        try {
            ...
            LOGGER.debug(" ***** TIMESTAMP WARNING ***** setting date to : " + databaseDate);
    ...
    public CalibrationConstants  readConstants(String table_name){
        ...
        for(int loop = 3; loop < ncolumns; loop++){
            //LOGGER.debug("COLUMN " + typecolumn.get(loop).getName()
        ...
        /*
                ...
                LOGGER.debug("VALUES SIZE = " + row.size());
    ...
    public void loadTable(String table_name){
        try {
            ...
            Vector<TypeTableColumn> typecolumn = asgmt.getTypeTable().getColumns();
            LOGGER.debug("[DB LOAD] ---> loading data table : " + table_name);
            LOGGER.debug("[DB LOAD] ---> number of columns  : " + typecolumn.size());
            for(int loop = 0; loop < ncolumns; loop++){
                //LOGGER.debug("Reading column number " + loop
                ...
                //LOGGER.debug(); //next line after a row
            }
            //provider.close();
        } catch (Exception e){
            LOGGER.debug("[DB LOAD] --->  error loading table : " + table_name);
            this.loadTimeErrors++;
        }
    }
    ...
    public void disconnect(){
        LOGGER.debug("[DB] --->  database disconnect  : success");
        this.provider.close();
    }
    /**
    ...
    public void show(){
        ...
        LOGGER.debug("\t" + StringTable.getCharacterString("*", 70));
        LOGGER.debug(String.format("\t*  %-52s : %8s   *", "Item Name","Length"));
        LOGGER.debug("\t" + StringTable.getCharacterString("*", 70));
    ...
    public String toString(){
        LOGGER.warn("Database Constat Provider: ");
        StringBuilder str = new StringBuilder();
        if(PRINT_ALL==true){
