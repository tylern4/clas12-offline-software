...
{
	public static Logger LOGGER = LogManager.getLogger(AlignmentFactory.class.getName());
		...
		for( int k = 0; k < dataMeasured.length/3; k+=3 ) // triangle
		{
			...
			for( int j = 0; j < 3+2; j++ ) // point, with offset x2
			{
				...
				if( j > 1 ) // offset to wait for both distanceDeltas of each vertex to be defined
				{
					...
					LOGGER.debug(" PD(% 8.3f % 8.3f)", pointDeltas[(j-2)*2+0], pointDeltas[(j-2)*2+1] );
					...
					//LOGGER.debug("(% 8.3f % 8.3f)", unitSideA.mag(), unitSideB.mag() );
					
					adjustments[j-2] = new Vector3d( unitSideA.add( unitSideB ) );
					adjustments[j-2].scale( 0.5 );
					LOGGER.debug("(% 8.3f)", adjustments[j-2].mag() );
			...
			LOGGER.debug("    ND  % 8.3f % 8.3f % 8.3f", dataNominalDistances[k][0], dataNominalDistances[k][1], dataNominalDistances[k][2] );
	...
	public static double[][] calcShifts( int dataLen, double[][] dataIdeal, double[][] dataMeasured )
	{
		...
		for( int k = 0; k < dataLen; k++ )
		{
			...
			if( VERBOSE )
			{
				...
				for( int f = 0; f < 3; f++ )
				{
					LOGGER.debug("IP%d % 8.3f % 8.3f % 8.3f", f, fidIdealPos3Ds[f].x, fidIdealPos3Ds[f].y, fidIdealPos3Ds[f].z );
					LOGGER.debug("    MP%d % 8.3f % 8.3f % 8.3f", f, fidMeasuredPos3Ds[f].x, fidMeasuredPos3Ds[f].y, fidMeasuredPos3Ds[f].z );
					LOGGER.debug("    D%d % 8.3f % 8.3f % 8.3f", f, fidDeltas[f].x, fidDeltas[f].y, fidDeltas[f].z );
					LOGGER.debug("");
				}
				LOGGER.debug("IC  % 8.3f % 8.3f % 8.3f", fidIdealCenPos3D.x, fidIdealCenPos3D.y, fidIdealCenPos3D.z );
				LOGGER.debug("    MC  % 8.3f % 8.3f % 8.3f", fidMeasuredCenPos3D.x, fidMeasuredCenPos3D.y, fidMeasuredCenPos3D.z );
				//LOGGER.debug("    DC  % 8.3f % 8.3f % 8.3f", fidDeltaCen.x, fidDeltaCen.y, fidDeltaCen.z );
				LOGGER.debug("");
				LOGGER.debug("IV  % 8.3f % 8.3f % 8.3f", fidIdealVec3D.x, fidIdealVec3D.y, fidIdealVec3D.z );
				LOGGER.debug("    MV  % 8.3f % 8.3f % 8.3f", fidMeasuredVec3D.x, fidMeasuredVec3D.y, fidMeasuredVec3D.z );
				LOGGER.debug("");
				LOGGER.debug("ST  % 8.3f % 8.3f % 8.3f", fidDiffVec3D.x, fidDiffVec3D.y, fidDiffVec3D.z );
				LOGGER.debug("    SR  % 8.3f % 8.3f % 8.3f % 8.3f", axisAngle[0], axisAngle[1], axisAngle[2], Math.toDegrees(axisAngle[3]) );
				LOGGER.debug("");
			}
	...
	public static double[][][] calcDeltas( int dataLen, int dataWid, int dataThk, double[][][] dataIdeal, double[][][] dataMeasured )
	{
		...
		for( int j = 0; j < dataLen; j++ )
		{
			...
			for( int i = 0; i < dataWid; i++ )
			{
				...
				if( VERBOSE ) LOGGER.debug("% 8.3f:% 8.3f - % 8.3f:% 8.3f = % 8.3f:% 8.3f\n",
	...
	public static double[][] applyShift( double[][] aData, double[][] aShift, double[][] aCenterData, double aScaleT, double aScaleR )
	{
		...
		for( int j = 0; j < aData.length/3; j+=3 )
			for( int i = 0; i < 3; i++ )
			{
				...
				if( VERBOSE ) LOGGER.debug("applying shift to point %d\n", i );
	...
	public static void applyShift( Vector3d aPoint, double[] aShift, Vector3d aCenter, double aScaleT, double aScaleR ) throws IllegalArgumentException
	{
		...
		// LOGGER.debug("Called applyShift. Shift[0][0]  = %8.4f ******************************************\n",tarray[0]);
		// LOGGER.debug("Called applyShift. Shift[0][1]  = %8.4f ******************************************\n",tarray[1]);
		// tarray = SVTConstants.getDataAlignmentSectorShift()[10];
		// LOGGER.debug("Called applyShift. Shift[0][10] = %8.4f ******************************************\n",tarray[0]);
		// tarray = SVTConstants.getDataAlignmentSectorShift()[65];
		// LOGGER.debug("Called applyShift. Shift[0][65] = %8.4f ******************************************\n",tarray[0]);
		...
		if( VERBOSE )
		{
			LOGGER.debug("PN: % 8.3f % 8.3f % 8.3f\n", aPoint.x, aPoint.y, aPoint.z );
			LOGGER.debug("ST: % 8.3f % 8.3f % 8.3f\n", tx, ty, tz );
			LOGGER.debug("SR: % 8.3f % 8.3f % 8.3f % 8.3f\n", rx, ry, rz, Math.toDegrees(ra) );
			LOGGER.debug("SC: % 8.3f % 8.3f % 8.3f\n", aCenter.x, aCenter.y, aCenter.z );
		}
		...
		if( VERBOSE ) LOGGER.debug("PS: % 8.3f % 8.3f % 8.3f\n", aPoint.x, aPoint.y, aPoint.z );
	}
	...
	public static void applyShift( Geant4Basic aVol, double[] aShift, Vector3d aNominalCenter, double aScaleT, double aScaleR ) throws IllegalArgumentException
	{
		...
		if( VERBOSE ) LOGGER.debug( aVol.gemcString() );
		...
		for( int i = 0; i < vr.length; i++ )
		{
			...
			LOGGER.debug("vector "+i+" matrix rotated"); Matrix.matMul( rotMatrix, new Matrix(3, 1, Util.toDoubleArray( vr[i] ) ) ).show();
		}*/
		...
		if( VERBOSE ) LOGGER.debug("RS: % 8.3f % 8.3f % 8.3f\n", Math.toDegrees(-rot[0]), Math.toDegrees(-rot[1]), Math.toDegrees(-rot[2]) );
